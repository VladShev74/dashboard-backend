...
async function getClassById(objectId) {
  try {
    ...
    console.error(
      "Something went wrong querying class with id " +
        objectId +
        "from DB: " +
        err
    );
...
app.get("/api/classes/:id", async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ error: "Request must contain an id query parameter" });
  }

  const fitness_class = await getClassById(id);
  if (!fitness_class)
    return res.status(404).json({ error: "Fitnes Class not found" });
